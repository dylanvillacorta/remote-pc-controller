...
type Delivery interface {
	Deliver(context.Context, Command) error
}
type Service struct {
	policy  Policy
	id      IDGenerator
...
func WithNonceGenerator(value NonceGenerator) ServiceOption {
	return func(s *Service) { s.nonce = value }
}
...
func NewService(policy Policy, options ...ServiceOption) (*Service, error) {
	...
	if policy.DeviceID == "" || policy.MaxValidity <= 0 || s.id == nil || s.nonce == nil || s.sign == nil || s.deliver == nil || s.now == nil {
		return nil, fmt.Errorf("command service: invalid configuration")
	}
	return s, nil
}
...
func (s *Service) Deliver(ctx context.Context, request Request) (Command, error) {
	c, err := s.Create(ctx, request)
	if err != nil {
		return Command{}, err
	}
	if err := s.deliver.Deliver(ctx, c); err != nil {
		return Command{}, fmt.Errorf("%w: %v", ErrDeliveryFailed, err)
	}
	return c, nil
}
