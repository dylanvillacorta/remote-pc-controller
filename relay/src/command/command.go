...
type Policy struct { DeviceID string; MaxValidity time.Duration }
...
func RandomNonce() (string, error) { b := make([]byte, 32); if _, err := rand.Read(b); err != nil { return "", err }; return base64.RawURLEncoding.EncodeToString(b), nil }
...
func Build(policy Policy, request Request, now time.Time, id IDGenerator, nonce NonceGenerator, sign Signer) (Command, error) {
	if err := policy.Validate(request); err != nil { return Command{}, err }
	if policy.MaxValidity <= 0 { return Command{}, ErrInvalidRequest }
	commandID, err := id(); if err != nil { return Command{}, fmt.Errorf("generate command id: %w", err) }
	nonceValue, err := nonce(); if err != nil { return Command{}, fmt.Errorf("generate nonce: %w", err) }
	...
	c.Signature, err = sign(c.Canonical()); if err != nil { return Command{}, fmt.Errorf("sign command: %w", err) }
	return c, nil
}
